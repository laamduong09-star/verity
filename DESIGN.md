...
The system reads as math worked out on graph paper, not a banking dashboard. A soft pastel gradient-mesh runs behind the whole page: blurred, low-opacity CSS blobs in a single blue hue (`--blue`/`--blue-focus`), drifting slowly and asymmetrically. Fixed to the viewport so it's visible behind every card all the way down the page, not just the hero. Layered above the mesh on the landing hero specifically: 42 small floating blue dots (`.hero-decor`), masked with a radial-gradient edge fade so they fade out toward the hero's edges rather than cutting off sharply, and centered on the "Money made easy." headline rather than the announcement chip above it. The landing page is the constellation's only home — module pages (including the calculator) keep their heroes clean. Surfaces are warm off-white paper and card tones, never cold navy-and-gray; text leans toward near-black ink rather than washed-out gray, so the whole thing stays legible and calm rather than corporate or clinical.
...
- **Rest state:** plain text links sitting directly on the page — no capsule chrome — in a 3-column grid (logo / nav / language toggle) so the nav stays optically centered, with the bar hugging the top edge (8px top padding) rather than floating below it. Active link gets a solid Constellation Blue boxy chip (`10px`); inactive links hover to a tall opaque card-white box (12px vertical padding, Amplemarket-style) — flipping to paper inside the compact capsule, whose own card-toned background would otherwise swallow it. Typography is Be Vietnam Pro 14px/500.
