...
- **The blue ambient background** — *removed 2026-07-19 (user-directed).* Its form evolved blob mesh (`.bg-mesh`) → restored by user request → aurora wash (`.bg-aurora`, 2026-07-10) → **removed entirely on every page 2026-07-19**; pages now open on flat cream with no ambient wash. Blue accordingly survives only inside data UI (chart, results, meters, focus rings, links — the Two-Accent-in-Data-UI Rule) and the custom cursor; there is no longer a decorative "theme is blue" ambient layer.
